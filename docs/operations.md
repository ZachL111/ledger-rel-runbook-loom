...
`ledger-rel-runbook-loom` is centered on reliability work in Solidity. The technical target is: Develop a Solidity command-oriented project for runbook scenarios with append-only fixtures, checkpoint recovery checks, and synthetic fixtures only.
